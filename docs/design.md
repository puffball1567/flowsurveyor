...
- Compare variants from observed event totals.
...
The v0.1.0 release does not promise data collection, persistent storage,
...
Variant comparison summarizes observed event count, duration, and failures per
variant. FlowSurveyor does not choose a winner by itself; callers can combine
the comparison with domain-specific cost or reliability rules.
...
FlowCaptain can later connect these components through adapters.
